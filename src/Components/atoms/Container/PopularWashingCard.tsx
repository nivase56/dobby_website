...
const PopularWashingCard = ({ data }: any) => {
  return (
    <div className="border rounded-lg w-[20%] mx-5 my-8 p-5 flex flex-col items-center justify-center">
      <Image
        src={data.imgsrc}
        alt={data.name}
        width={200}
      ...
      />
      <h3 className="py-2 font-semibold">{data.name}</h3>
      <div className="flex items-center gap-3">
        <IconComponent
          icon="noto:star"
          className="text-primary h-4 w-4 text-gray-500"
        />
        <p className="flex gap-1">{data.rating} <p className="font-semibold">({data.count} reviews)</p></p>
      </div>
